...
Dbpedia_category_list_to_filter = [
    "DBpedia:MusicalWork", "DBpedia:MusicalArtist",
    "Schema:MusicGroup", "Schema:MusicAlbum", "Schema:MusicRecording",
    "DBpedia:Film", "DBpedia:MusicGenre", "Memory_management",
    "DBpedia:Device", "DBpedia:InformationAppliance", "Schema:CreativeWork",
    "DBpedia:TelevisionShow", "DBpedia:Software", "DBpedia:VideoGame",
    "DBpedia:Magazine", "DBpedia:BroadcastNetwork", "DBpedia:Company"
]
    ...
    for paragraph in paragraphs:
        if paragraph.text:
            try:
                ...
                for ent in doc.ents:
                    # Obtenez les types DBpedia de l'entité
                    entity_types = ent._.dbpedia_raw_result.get('@types', '').split(',')
                    # Vérifier si l'entité est dans les catégories ou liens à filtrer
                    if not any(ex_type in entity_types for ex_type in Dbpedia_category_list_to_filter) and \
                       not any(link in ent.kb_id_ for link in link_dbpedia_to_filter):
                        # Créer la note et l'ajouter au paragraphe
